...
function toUnit(meters: number, unit: Unit) {
  return unit === "km" ? meters / 1000 : meters / 1609.344;
}
...
export default function FitnessAreaChart({
  ...
  const data = useMemo(() => {
    ...
    for (const a of activities) {
      ...
      buckets[k].dist += toUnit(a.distance_meters ?? 0, unit);
      buckets[k].count += 1;
    }
    ...
    return Object.entries(buckets)
      ...
      .map(([date, v]) => ({
        date,
        distance: parseFloat(v.dist.toFixed(1)),
        count: v.count,
      }));
