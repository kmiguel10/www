...
function StatCard({
  ...
  return (
    <div className="flex flex-col justify-between rounded-xl border border-gray-6 bg-gray-2 p-4">
      <div className="text-xs font-medium uppercase tracking-widest text-gray-11">
        {label}
      </div>
...
function SectionCard({
  ...
  return (
    <div
      className={clsx(
        "flex w-full flex-col overflow-hidden rounded-xl border border-gray-6 bg-gray-2",
        className
      )}
    >
...
export default function FitnessDashboard({
  ...
  const sportTypes = useMemo(() => {
    const types = new Set(
      activities.map((a) => a.sport_type).filter(Boolean) as string[]
    );
    ...
    <div className="flex flex-col space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <PillButton
            active={source === "strava"}
            onClick={() => setSource("strava")}
            activeClass="bg-orange-900 border-orange-700 text-orange-300"
          >
            Strava
          </PillButton>
          <PillButton
            active={source === "garmin"}
            onClick={() => setSource("garmin")}
            activeClass="bg-blue-900 border-blue-700 text-blue-300"
          >
            Garmin
          </PillButton>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {sportTypes.map((t) => (
            <PillButton
              ...
              size="sm"
              active={sportType === t}
              onClick={() => setSportType(sportType === t ? "all" : t)}
            >
              {SPORT_LABELS[t] ?? t}
            </PillButton>
          ))}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <StatCard
          label="Distance"
          value={fmtDistance(stats.distance, unit)}
        />
