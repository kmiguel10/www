...
import type { FitnessActivity } from "../page";

// Radix dark mode palette values
const SPORT_COLORS: Record<string, string> = {
  ...
  Ride: "#30A46C",
  VirtualRide: "#30A46C",
  MountainBikeRide: "#30A46C",
  GravelRide: "#30A46C",
  EBikeRide: "#30A46C",
  Walk: "#F76B15",
  Hike: "#F76B15",
  Swim: "#00BAD3",
  WeightTraining: "#8E4EC6",
  Workout: "#8E4EC6",
  Yoga: "#E54D2E",
};
...
const SPORT_LABELS: Record<string, string> = {
  ...
  GravelRide: "Gravel", EBikeRide: "E-Bike",
...
export default function FitnessBarChart({ activities }: { activities: FitnessActivity[] }) {
  ...
  const data = useMemo(() => {
    ...
    // Weekly buckets: { [week]: { [sport]: count } }
    ...
    return Object.entries(buckets)
      ...
      .map(([date, sports]) => {
        // Merge into a flat array of { date, sport, count } for stacked rendering
        const entries = Object.entries(sports);
        return { date, sports: entries, total: entries.reduce((s, [, v]) => s + v, 0) };
      });
  }, [activities, days]);

  // For a simpler approach: show total activities per week as a single bar,
  // colored by the most common sport that week
  const chartData = useMemo(
    () =>
      data.map((row) => {
        const topSport =
          row.sports.length > 0
            ? row.sports.sort((a, b) => b[1] - a[1])[0][0]
            : "Other";
        return {
          date: row.date,
          total: row.total,
          ...
          <Tooltip
            ...
            content={({ active, payload, label }) =>
              active && payload?.length ? (
                <div className="rounded border border-gray-6 bg-gray-3 p-2 text-sm min-w-[140px]">
                  ...
                  {payload[0]?.payload?.breakdown?.map(([sport, count]: [string, number]) => (
                    <div key={sport} className="flex items-center gap-2 py-0.5">
                      <span
                        ...
                        style={{ background: SPORT_COLORS[sport] ?? DEFAULT_COLOR }}
                      />
