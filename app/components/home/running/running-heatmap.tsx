...
const RunningFeatureDetailHeatmap: FC<RunningFeatureDetailHeatmapProps> = ({
  ...
  const yearsLogged = useMemo(
    () =>
      Array.from(
        new Set(runningLogs.map((log) => new Date(log.date).getFullYear()))
      )
        .sort()
        .reverse(),
    [runningLogs]
    ...
    (event: PointerEvent<SVGSVGElement>) => {
      ...
      const tooltipData =
        dataDate !== undefined && dataValue !== undefined
          ? JSON.stringify({
              date: dataDate,
              value: formatValueToPrecision(
                unit.scalar * Number(dataValue),
                2,
                false
              ),
            })
  ...
  const logs = useMemo(() => {
    ...
    for (let i = 0; i < filteredLogs.length; ++i) {
      ...
      days[currentUTCDate.getUTCDay()][
        Math.floor(i / 7) + (date.getUTCDay() < dayOffset ? 1 : 0)
  ...
  const max = useMemo(
    () => Math.max(...filteredLogs.map((log) => log.value)),
    [filteredLogs]
  );
  const total = useMemo(
