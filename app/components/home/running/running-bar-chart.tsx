...
export default function RunningBarChart({
  ...
  const total = useMemo(
    () =>
      (365 * runMonthlyData.reduce((a, b) => a + unit.scalar * b.value, 0)) /
      totalDays,
  ...
  useEffect(() => {
    const yAxis = document.getElementsByClassName(
      "recharts-cartesian-axis recharts-yAxis"
    )[0];
    setYAxisWidth(
      yAxis
        ? (yAxis as SVGGraphicsElement)?.getBoundingClientRect().width + 4
        : 20.43
    );
  }, [data]);
  ...
  return (
    <Fragment>
      <div className="font-medium">
        <Tooltip
          content={`${formatValueToPrecision(
            total / 365,
            2,
            false
          )}${unitName}/day`}
      ...
      <div className="mt-0.5 text-xs text-gray-11">
        {data.length > 0
          ? `${data[0].date.toLocaleDateString("en-US", {
              month: "short",
              timeZone: "UTC",
            })} ${data[0].date.getUTCFullYear()} to ${data[
              data.length - 1
